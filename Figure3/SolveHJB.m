...
v=@(x) 1+0.25*sin(2*x(1))*sin(2*x(2));
gv = @(x) 0.5*[cos(2*x(1))*sin(2*x(2));sin(2*x(1))*cos(2*x(2))];
...
for k = 1:max_iter
    ...
    for j = 2:J
        ...
        if isempty(r)
                ...
                [D,m] = min((x1(1,j)-xCt(1,:)).^2 + (x1(2,j)-xCt(2,:)).^2-r.^2);
                O1(j) = 1/2 + (1/2)*tanh(B*D);
                gO1(:,j) = 2*B*sech(B*D)^2*(x1(:,j)-xCt(:,m));
                [D,m] = min((x2(1,j)-xCt(1,:)).^2 + (x2(2,j)-xCt(2,:)).^2-r.^2);
                O2(j) = 1/2 + (1/2)*tanh(B*D);
                gO2(:,j) = 2*B*sech(B*D)^2*(x2(:,j)-xCt(:,m));
                [D,m] = min((x3(1,j)-xCt(1,:)).^2 + (x3(2,j)-xCt(2,:)).^2-r.^2);
                O3(j) = 1/2 + (1/2)*tanh(B*D);
                gO3(:,j) = 2*B*sech(B*D)^2*(x3(:,j)-xCt(:,m));
        end
